...
control SwitchIngress(
    ...
    //Pkt_Register
    Register<bit<32>,bit<16>>(REGISTER_SIZE,0) Pkt_Register;
    RegisterAction<bit<32>,bit<16>, bool>(Pkt_Register) Pkt_Register_action = {
        void apply(inout bit<32> value, out bool flag) {
            value = value + ig_md.data_num;
            flag=value>ig_md.PACKET_THRESHOLD;
        }
    };
    //Pkt_Register_01
    Register<bit<8>,bit<16>>(REGISTER_SIZE,0) Pkt_Register_01;
    RegisterAction<bit<8>,bit<16>, bool>(Pkt_Register_01) Pkt_Register_action_01 = {
        void apply(inout bit<8> value, out bool flag) {
            flag=false;
            if(value==0)
            {flag=true;
            value = 1;}
    ...
    apply {
        ...
        {
            //过滤过载端口，防止同一端口多次记录
            local_flag_01=Pkt_Register_action_01.execute(ig_md.index);
        }
        ...
        // 第二层检测
        if (local_flag_01) {
            local_flag_02=Port_Register_action.execute(egress_port);
        }
